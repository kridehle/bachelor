...
def lag_IQ_data(args, I_signal, Q_signal):
    ...
    if args.datatype == 'f':
        ...
        print(f"IQ data blir lagret som float32")
        ...
        print(f"IQ data blir lagret som int16")
        int_skalar = 32767
...
def skriv_IQ_data(IQ_data,args):
    filnavn = f"{args.fil}_{args.samplingsfrekvens}MHz_{args.datatype}.bin" if args.fil else "iq_data.bin"
    #Skriver IQ data til fil som skrives til i binær modus. (write binary)
    with open (filnavn, "wb") as fil:
        fil.write(IQ_data)

    print(f"\nIQ-data lagret til: {filnavn}")
...
def plott_resultat(bølge_variabler,args):
    ...
    print(f"\nEn grafisk fil er lagret til: grafisk_ut.png")

    # Plot valideringsbølge på andre subplot (nederste rutenett)
    axs[0].plot(tidsvektor, valideringsbølge, color='b', label='Valideringsbølge')
    axs[0].set_title("Valideringsbølge")
    ...
    axs[1].plot(tidsvektor, I, color='green', label='I')
    axs[1].plot(tidsvektor, Q, color='orange', label='Q')
    axs[1].set_title("I og Q data")
    ...
    axs[2].plot(frekvenser[:N//10], magnitude[:N//10], color = 'b') 
    axs[2].set_title("Frekvensdomene")
    axs[2].set_xlabel("Frekvens [MHz]")
